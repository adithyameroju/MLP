import { CheckCircle, Pencil, Trash2, Wallet } from 'lucide-react'
import { useNavigate } from 'react-router-dom'
import { writeCdDraftImpact } from '../data/cdWalletMock'
...
export default function QuickAddReviewScreen({
  ...
  const openCdBalanceWithDraft = () => {
    writeCdDraftImpact({
      projectedBalanceAfter: cdAfterSubmit,
      estimatedDraw: estimatedCdDraw,
      sourceLabel: 'Quick Add preview',
      ...
      <div className="flex-1 min-h-0 flex flex-col lg:flex-row gap-4 lg:gap-5 lg:items-stretch min-h-0 overflow-hidden">
        ...
        <aside className="w-full lg:w-[min(calc(19rem+10px),32vw)] lg:max-w-[calc(21rem+10px)] shrink-0 order-2 lg:min-h-0 flex flex-col lg:justify-start">
          <div className="w-full lg:max-h-[min(calc(100vh-8rem),40rem)] lg:overflow-y-auto overscroll-contain pb-2">
            ...
            <button
              type="button"
              onClick={openCdBalanceWithDraft}
              className="mt-2 w-full inline-flex items-center justify-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50/80 py-2 text-[11px] font-semibold text-indigo-800 hover:bg-indigo-100/90 cursor-pointer"
            >
              <Wallet size={13} aria-hidden />
              Open CD balance with this estimate
            </button>
          </div>
        </aside>
      </div>
