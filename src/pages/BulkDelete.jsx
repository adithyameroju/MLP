...
export default function BulkDelete() {
  ...
  const handleSubmit = () => {
    setUploading(true)
    addEntry({ action: 'Bulk Upload - Delete', count: 0, status: 'In Progress', type: 'bulk' })
    setTimeout(() => {
      setUploading(false)
      setDone(true)
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-lg font-bold text-gray-900 mb-1">Processing Deletion</h2>
          <p className="text-sm text-gray-500">Your deletion file is being validated...</p>
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-lg font-bold text-gray-900 mb-1">Deletion Submitted</h2>
  ...
  return (
    <div className="h-full overflow-y-auto px-6 lg:px-8 py-3">
      <PageHeader
        title="Bulk Deletion"
        subtitle="Upload a file with employee IDs to remove in bulk"
        breadcrumbs={[
          endorsementsModuleCrumb,
          { label: 'Delete Employee', path: '/delete' },
          { label: 'Bulk Deletion' },
        ]}
        trailing={
      ...
      <div className="space-y-5">
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3.5">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-red-900">Download the deletion template</p>
            <p className="text-xs text-red-700 leading-relaxed mt-0.5">Fill in employee IDs and upload to remove them in bulk.</p>
            <button className="mt-2.5 inline-flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-medium text-red-700 bg-white border border-red-200 rounded-lg hover:bg-red-50 cursor-pointer"><Download size={14} /> Download Template</button>
          </div>
        </div>
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-xl py-12 px-8 flex flex-col items-center justify-center cursor-pointer transition-colors ${
            dragging ? 'border-red-400 bg-red-50' : file ? 'border-emerald-300 bg-emerald-50/30' : 'border-gray-300 bg-white hover:border-gray-400'
          }`}
        >
        ...
        <div className="flex justify-end gap-3">
          ...
          <button disabled={!file} onClick={handleSubmit} className="px-5 py-2.5 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-2 cursor-pointer">
            <CheckCircle size={16} /> Submit Deletion
          </button>
        </div>
      </div>
    </div>
